...
with open('./pilot_path.json') as file:
    data = json.load(file)
    print(data)
print('---------------')
...
# ВАШ КОД:
pilot_missions_count = dict()
for pilot in data:
    pilot_missions_count[pilot] = len(data[pilot]['missions'])
...
print(f"Пилоты в порядке убывания количества миссий: {dict(sorted(pilot_missions_count.items(), key=lambda item: item[1], reverse=True))}")
print('---------------')
...
# ВАШ КОД:
drones_in_missions = set()
for pilot in data:
    for mission in data[pilot]['missions']:
        drones_in_missions.add(mission['drone'])
# вывод результата (допишите код)
print(f'Полеты совершались на дронах следующих моделей: {", ".join(drones_in_missions)}')
print('---------------')
...
# ВАШ КОД:
drones_missions_count = dict()
for pilot in data:
    for mission in data[pilot]['missions']:
        drone = mission['drone']
        drones_missions_count[drone] = drones_missions_count.get(drone, 0) + 1
# вывод результата (допишите код)
for drone, missions_count in drones_missions_count.items():
...
class UAV:
	...
	@missions.setter
	def missions(self, missions_):
		self._missions.append(missions_)
...
for drone in drones_in_missions:
    multirotor_uav = MultirotorUAV(
    ...
    for pilot in data:
        for mission in data[pilot]['missions']:
            if mission['drone'] == drone:
                multirotor_uav.missions.append(mission['mission'])
    multirotor_uavs.append(multirotor_uav)
...
for drone in multirotor_uavs:
	if drone.get_model().lower() == user_input.lower():
